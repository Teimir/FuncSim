...
- **`CPUState`:** 32 GPR, слово флагов, SPR, признак `halted`. **R0** читается как ноль; **R31** — указатель команд (PC/IP).
...
| Флаги | `ADDS`/`SUBS` обновляют Z, C, V, S; условные переходы читают их из `CPUState.flags` |
...
Точка входа пакета: `e32c-debug-gui` → `cli.debug_gui:main`.

## 9. Зависимости и тесты

- **Python ≥ 3.10**, зависимость времени выполнения: **PyYAML** (таблица опкодов).
- **pytest**, **hypothesis**, **ruff** — опционально в `[dev]` ([pyproject.toml](../pyproject.toml)).
- Тесты: `tests/` — ISA, периферия, шина, интеграционные сценарии.
