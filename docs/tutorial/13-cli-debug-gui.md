...
**Смена ядра без перезапуска:** выпадающий список **Core** на панели инструментов или **View → Core variant** (`full` / `tn9k` / `lite`). После смены выполняется сброс CPU (breakpoints сохраняются); обновляются SPR `CORE_INFO` / `FEATURES` и набор illegal-опкодов.

Флаг `--core` при запуске задаёт начальный профиль.
