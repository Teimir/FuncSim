//===-- E32CISelLowering.cpp - E32C DAG Lowering Implementation ---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the E32CTargetLowering class.
//
//===----------------------------------------------------------------------===//
...
static cl::opt<int> E32CLowerConstantMulThreshold(
    "e32c-constant-mul-threshold", cl::Hidden,
...
E32CTargetLowering::E32CTargetLowering(const TargetMachine &TM,
                                         const E32CSubtarget &STI)
    : TargetLowering(TM) {
  // Set up the register classes.
  addRegisterClass(MVT::i32, &E32C::GPRRegClass);

  // Compute derived properties from the register classes
  TRI = STI.getRegisterInfo();
  computeRegisterProperties(TRI);

  setStackPointerRegisterToSaveRestore(E32C::SP);
...
SDValue E32CTargetLowering::LowerOperation(SDValue Op,
...
//===----------------------------------------------------------------------===//
//                       E32C Inline Assembly Support
//===----------------------------------------------------------------------===//

Register E32CTargetLowering::getRegisterByName(
...
E32CTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1)
    ...
    switch (Constraint[0]) {
    case 'r': // GENERAL_REGS
      return std::make_pair(0U, &E32C::GPRRegClass);
    default:
      break;
    }
...
E32CTargetLowering::getSingleConstraintMatchWeight(
...
void E32CTargetLowering::LowerAsmOperandForConstraint(
...
#include "E32CGenCallingConv.inc"

static unsigned NumFixedArgs;
static bool CC_E32C32_VarArg(unsigned ValNo, MVT ValVT, MVT LocVT,
  ...
  if (ValNo < NumFixedArgs) {
    return CC_E32C32(ValNo, ValVT, LocVT, LocInfo, ArgFlags, State);
  }
...
SDValue E32CTargetLowering::LowerFormalArguments(
...
SDValue E32CTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // E32C target does not yet support tail call optimization.
  IsTailCall = false;
...
SDValue E32CTargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  ...
  E32CMachineFunctionInfo *E32CMFI = MF.getInfo<E32CMachineFunctionInfo>();
  ...
  if (CallConv == CallingConv::Fast) {
    CCInfo.AnalyzeFormalArguments(Ins, CC_E32C32_Fast);
  } else {
    CCInfo.AnalyzeFormalArguments(Ins, CC_E32C32);
  }
  ...
  for (const CCValAssign &VA : ArgLocs) {
    if (VA.isRegLoc()) {
      ...
      case MVT::i32: {
        Register VReg = RegInfo.createVirtualRegister(&E32C::GPRRegClass);
  ...
  // The E32C ABI for returning structs by value requires that we copy
  // the sret argument into rv for the return. Save the argument into
  // a virtual register so that we can access it from the return points.
  if (MF.getFunction().hasStructRetAttr()) {
    Register Reg = E32CMFI->getSRetReturnReg();
    if (!Reg) {
      Reg = MF.getRegInfo().createVirtualRegister(getRegClassFor(MVT::i32));
      E32CMFI->setSRetReturnReg(Reg);
    }
  ...
  if (IsVarArg) {
    ...
    E32CMFI->setVarArgsFrameIndex(FI);
  }

  return Chain;
}

bool E32CTargetLowering::CanLowerReturn(
    CallingConv::ID CallConv, MachineFunction &MF, bool IsVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs, LLVMContext &Context) const {
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, RVLocs, Context);

  return CCInfo.CheckReturn(Outs, RetCC_E32C32);
}

SDValue
E32CTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                 bool IsVarArg,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_E32C32);
  ...
  if (DAG.getMachineFunction().getFunction().hasStructRetAttr()) {
    ...
    Chain = DAG.getCopyToReg(Chain, DL, E32C::R0, Val, Glue);
    Glue = Chain.getValue(1);
    RetOps.push_back(
        DAG.getRegister(E32C::R0, getPointerTy(DAG.getDataLayout())));
  }

  RetOps[0] = Chain; // Update chain

  unsigned Opc = E32CISD::RET_GLUE;
  if (Glue.getNode())
...
SDValue E32CTargetLowering::LowerCCCCallTo(
  ...
  if (NumFixedArgs)
    CCInfo.AnalyzeCallOperands(Outs, CC_E32C32_VarArg);
  else {
    if (CallConv == CallingConv::Fast)
      CCInfo.AnalyzeCallOperands(Outs, CC_E32C32_Fast);
    else
      CCInfo.AnalyzeCallOperands(Outs, CC_E32C32);
  }
  ...
  for (unsigned I = 0, J = 0, E = ArgLocs.size(); I != E; ++I) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (StackPtr.getNode() == nullptr)
        StackPtr = DAG.getCopyFromReg(Chain, DL, E32C::SP,
  ...
  uint8_t OpFlag = E32CII::MO_NO_FLAG;
  if (G) {
  ...
  Chain = DAG.getNode(E32CISD::CALL, DL, NodeTys,
...
SDValue E32CTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_E32C32);
...
static LPCC::CondCode IntCondCCodeToICC(SDValue CC, const SDLoc &DL,
  ...
  // and E32C only supports integer comparisons, so only provide definitions
  // for them.
  switch (SetCCOpcode) {
  case ISD::SETEQ:
    return LPCC::ICC_EQ;
...
SDValue E32CTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue Glue =
      DAG.getNode(E32CISD::SET_FLAG, DL, MVT::Glue, LHS, RHS, TargetCC);

  return DAG.getNode(E32CISD::BR_CC, DL, Op.getValueType(), Chain, Dest,
                     TargetCC, Glue);
}

SDValue E32CTargetLowering::LowerMUL(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (InstrRequired > E32CLowerConstantMulThreshold)
    return SDValue();
...
SDValue E32CTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(E32CISD::SETCC, DL, Op.getValueType(), TargetCC, Glue);
}

SDValue E32CTargetLowering::LowerSELECT_CC(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  ...
  return DAG.getNode(E32CISD::SELECT_CC, DL, VTs, TrueV, FalseV, TargetCC,
                     Glue);
}

SDValue E32CTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  E32CMachineFunctionInfo *FuncInfo = MF.getInfo<E32CMachineFunctionInfo>();
...
SDValue E32CTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDValue Chain = Op.getOperand(0);
  ...
  // For E32C, the outgoing memory arguments area should be on top of the
  ...
  SDValue ArgAdjust = DAG.getNode(E32CISD::ADJDYNALLOC, DL, MVT::i32, Sub);
...
SDValue E32CTargetLowering::LowerRETURNADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
SDValue E32CTargetLowering::LowerFRAMEADDR(SDValue Op,
                                            SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
  ...
  EVT VT = Op.getValueType();
  SDLoc DL(Op);
  SDValue FrameAddr = DAG.getCopyFromReg(DAG.getEntryNode(), DL, E32C::SP, VT);
  unsigned Depth = Op.getConstantOperandVal(0);
  while (Depth--) {
...
const char *E32CTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case E32CISD::ADJDYNALLOC:
    return "E32CISD::ADJDYNALLOC";
  case E32CISD::RET_GLUE:
    return "E32CISD::RET_GLUE";
  case E32CISD::CALL:
    return "E32CISD::CALL";
  case E32CISD::SELECT_CC:
    return "E32CISD::SELECT_CC";
  case E32CISD::SETCC:
    return "E32CISD::SETCC";
  case E32CISD::SUBBF:
    return "E32CISD::SUBBF";
  case E32CISD::SET_FLAG:
    return "E32CISD::SET_FLAG";
  case E32CISD::BR_CC:
    return "E32CISD::BR_CC";
  case E32CISD::Wrapper:
    return "E32CISD::Wrapper";
  case E32CISD::HI:
    return "E32CISD::HI";
  case E32CISD::LO:
    return "E32CISD::LO";
  case E32CISD::SMALL:
    return "E32CISD::SMALL";
  default:
    return nullptr;
  }
}

SDValue E32CTargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ConstantPoolSDNode *N = cast<ConstantPoolSDNode>(Op);
  const Constant *C = N->getConstVal();
  const E32CTargetObjectFile *TLOF =
      static_cast<const E32CTargetObjectFile *>(
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small ||
    ...
    SDValue Small = DAG.getTargetConstantPool(
        C, MVT::i32, N->getAlign(), N->getOffset(), E32CII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(E32C::R0, MVT::i32),
                       DAG.getNode(E32CISD::SMALL, DL, MVT::i32, Small));
  } else {
    uint8_t OpFlagHi = E32CII::MO_ABS_HI;
    uint8_t OpFlagLo = E32CII::MO_ABS_LO;
    ...
    Hi = DAG.getNode(E32CISD::HI, DL, MVT::i32, Hi);
    Lo = DAG.getNode(E32CISD::LO, DL, MVT::i32, Lo);
    SDValue Result = DAG.getNode(ISD::OR, DL, MVT::i32, Hi, Lo);
    return Result;
  }
}

SDValue E32CTargetLowering::LowerGlobalAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (TLOF->isGlobalInSmallSection(GO, getTargetMachine())) {
    SDValue Small = DAG.getTargetGlobalAddress(
        GV, DL, getPointerTy(DAG.getDataLayout()), Offset, E32CII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(E32C::R0, MVT::i32),
                       DAG.getNode(E32CISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue E32CTargetLowering::LowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue E32CTargetLowering::LowerJumpTable(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small) {
    SDValue Small = DAG.getTargetJumpTable(
        JT->getIndex(), getPointerTy(DAG.getDataLayout()), E32CII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(E32C::R0, MVT::i32),
                       DAG.getNode(E32CISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue E32CTargetLowering::LowerSHL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  ...
  // E32C shifts of larger than register sizes are wrapped rather than
...
SDValue E32CTargetLowering::LowerSRL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  ...
  // Note: This takes advantage of E32C's shift behavior to avoid needing to
...
SDValue E32CTargetLowering::PerformDAGCombine(SDNode *N,
...
void E32CTargetLowering::computeKnownBitsForTargetNode(
