//===-- E32CInstrInfo.cpp - E32C Instruction Information ------*- C++ -*-===//
//
...
E32CInstrInfo::E32CInstrInfo()
    : E32CGenInstrInfo(E32C::ADJCALLSTACKDOWN, E32C::ADJCALLSTACKUP),
      RegisterInfo() {}

void E32CInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
                                 MachineBasicBlock::iterator Position,
                                 const DebugLoc &DL,
                                 MCRegister DestinationRegister,
                                 MCRegister SourceRegister,
                                 bool KillSource) const {
  if (!E32C::GPRRegClass.contains(DestinationRegister, SourceRegister)) {
    llvm_unreachable("Impossible reg-to-reg copy");
  }

  BuildMI(MBB, Position, DL, get(E32C::OR_I_LO), DestinationRegister)
      .addReg(SourceRegister, getKillRegState(KillSource))
      .addImm(0);
}

void E32CInstrInfo::storeRegToStackSlot(
  ...
  if (!E32C::GPRRegClass.hasSubClassEq(RegisterClass)) {
    llvm_unreachable("Can't store this register to stack slot");
  }
  BuildMI(MBB, Position, DL, get(E32C::SW_RI))
...
void E32CInstrInfo::loadRegFromStackSlot(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator Position,
  ...
  BuildMI(MBB, Position, DL, get(E32C::LDW_RI), DestinationRegister)
      .addFrameIndex(FrameIndex)
      .addImm(0)
      .addImm(LPAC::ADD);
}

bool E32CInstrInfo::areMemAccessesTriviallyDisjoint(
...
bool E32CInstrInfo::expandPostRAPseudo(MachineInstr & /*MI*/) const {
  return false;
}
...
E32CInstrInfo::decomposeMachineOperandsTargetFlags(unsigned TF) const {
  return std::make_pair(TF, 0u);
}

ArrayRef<std::pair<unsigned, const char *>>
E32CInstrInfo::getSerializableDirectMachineOperandTargetFlags() const {
  using namespace E32CII;
...
bool E32CInstrInfo::analyzeCompare(const MachineInstr &MI, Register &SrcReg,
                                    Register &SrcReg2, int64_t &CmpMask,
                                    int64_t &CmpValue) const {
  switch (MI.getOpcode()) {
  default:
    break;
  case E32C::SFSUB_F_RI_LO:
  case E32C::SFSUB_F_RI_HI:
    SrcReg = MI.getOperand(0).getReg();
    SrcReg2 = Register();
    CmpMask = ~0;
    CmpValue = MI.getOperand(1).getImm();
    return true;
  case E32C::SFSUB_F_RR:
    SrcReg = MI.getOperand(0).getReg();
...
inline static bool isRedundantFlagInstr(MachineInstr *CmpI, unsigned SrcReg,
                                        ...
                                        MachineInstr *OI) {
  if (CmpI->getOpcode() == E32C::SFSUB_F_RR &&
      OI->getOpcode() == E32C::SUB_R &&
  ...
  if (((CmpI->getOpcode() == E32C::SFSUB_F_RI_LO &&
        OI->getOpcode() == E32C::SUB_I_LO) ||
       (CmpI->getOpcode() == E32C::SFSUB_F_RI_HI &&
        OI->getOpcode() == E32C::SUB_I_HI)) &&
      OI->getOperand(1).getReg() == SrcReg &&
...
inline static unsigned flagSettingOpcodeVariant(unsigned OldOpcode) {
  switch (OldOpcode) {
  case E32C::ADD_I_HI:
    return E32C::ADD_F_I_HI;
  case E32C::ADD_I_LO:
    return E32C::ADD_F_I_LO;
  case E32C::ADD_R:
    return E32C::ADD_F_R;
  case E32C::ADDC_I_HI:
    return E32C::ADDC_F_I_HI;
  case E32C::ADDC_I_LO:
    return E32C::ADDC_F_I_LO;
  case E32C::ADDC_R:
    return E32C::ADDC_F_R;
  case E32C::AND_I_HI:
    return E32C::AND_F_I_HI;
  case E32C::AND_I_LO:
    return E32C::AND_F_I_LO;
  case E32C::AND_R:
    return E32C::AND_F_R;
  case E32C::OR_I_HI:
    return E32C::OR_F_I_HI;
  case E32C::OR_I_LO:
    return E32C::OR_F_I_LO;
  case E32C::OR_R:
    return E32C::OR_F_R;
  case E32C::SL_I:
    return E32C::SL_F_I;
  ...
  case E32C::SRA_R:
    return E32C::SRA_F_R;
  case E32C::SUB_I_HI:
    return E32C::SUB_F_I_HI;
  case E32C::SUB_I_LO:
    return E32C::SUB_F_I_LO;
  case E32C::SUB_R:
    return E32C::SUB_F_R;
  case E32C::SUBB_I_HI:
    return E32C::SUBB_F_I_HI;
  case E32C::SUBB_I_LO:
    return E32C::SUBB_F_I_LO;
  case E32C::SUBB_R:
    return E32C::SUBB_F_R;
  case E32C::XOR_I_HI:
    return E32C::XOR_F_I_HI;
  case E32C::XOR_I_LO:
    return E32C::XOR_F_I_LO;
  case E32C::XOR_R:
    return E32C::XOR_F_R;
  default:
    return E32C::NOP;
...
bool E32CInstrInfo::optimizeCompareInstr(
  ...
  if (SrcReg2 != 0)
    ...
    if (CmpInstr.getOpcode() == E32C::SFSUB_F_RI_LO)
      MI = nullptr;
    else
      return false;
  }
  ...
  for (--I; I != E; --I) {
    ...
    if (Instr.modifiesRegister(E32C::SR, TRI) ||
        Instr.readsRegister(E32C::SR, TRI))
  ...
  if (flagSettingOpcodeVariant(MI->getOpcode()) != E32C::NOP) {
    bool isSafe = false;
    ...
    while (!isSafe && ++I != E) {
      ...
      for (unsigned IO = 0, EO = Instr.getNumOperands(); !isSafe && IO != EO;
        ...
        if (MO.isRegMask() && MO.clobbersPhysReg(E32C::SR)) {
          isSafe = true;
          break;
        }
        if (!MO.isReg() || MO.getReg() != E32C::SR)
          continue;
        if (MO.isDef()) {
          isSafe = true;
          break;
...
bool E32CInstrInfo::analyzeSelect(const MachineInstr &MI,
                                   SmallVectorImpl<MachineOperand> &Cond,
                                   unsigned &TrueOp, unsigned &FalseOp,
                                   bool &Optimizable) const {
  assert(MI.getOpcode() == E32C::SELECT && "unknown select instruction");
  // Select operands:
  // 0: Def.
  // 1: True use.
  // 2: False use.
...
MachineInstr *
E32CInstrInfo::optimizeSelect(MachineInstr &MI,
...
bool E32CInstrInfo::analyzeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->getOpcode() == E32C::BT) {
      if (!AllowModify) {
...
// reversed.
bool E32CInstrInfo::reverseBranchCondition(
    SmallVectorImpl<llvm::MachineOperand> &Condition) const {
  assert((Condition.size() == 1) &&
         "E32C branch conditions should have one component.");
...
unsigned E32CInstrInfo::insertBranch(MachineBasicBlock &MBB,
                                      MachineBasicBlock *TrueBlock,
                                      MachineBasicBlock *FalseBlock,
  ...
  if (Condition.empty()) {
    ...
    BuildMI(&MBB, DL, get(E32C::BT)).addMBB(TrueBlock);
    return 1;
  }
  ...
  BuildMI(&MBB, DL, get(E32C::BRCC)).addMBB(TrueBlock).addImm(ConditionalCode);
  ...
  BuildMI(&MBB, DL, get(E32C::BT)).addMBB(FalseBlock);
  return 2;
}

unsigned E32CInstrInfo::removeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->isDebugInstr())
      continue;
    if (Instruction->getOpcode() != E32C::BT &&
        Instruction->getOpcode() != E32C::BRCC) {
      break;
    }
...
Register E32CInstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
                                             int &FrameIndex) const {
  if (MI.getOpcode() == E32C::LDW_RI)
...
Register E32CInstrInfo::isLoadFromStackSlotPostFE(const MachineInstr &MI,
                                                   int &FrameIndex) const {
  if (MI.getOpcode() == E32C::LDW_RI) {
    unsigned Reg;
...
Register E32CInstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                            int &FrameIndex) const {
  if (MI.getOpcode() == E32C::SW_RI)
...
bool E32CInstrInfo::getMemOperandWithOffsetWidth(
...
bool E32CInstrInfo::getMemOperandsWithOffsetWidth(
