//===-- E32CInstPrinter.cpp - Convert E32C MCInst to asm syntax ---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This class prints an E32C MCInst to a .s file.
//
//===----------------------------------------------------------------------===//

#include "E32CInstPrinter.h"
#include "E32CMCExpr.h"
...
#include "E32CGenAsmWriter.inc"

void E32CInstPrinter::printRegName(raw_ostream &OS, MCRegister Reg) const {
  OS << StringRef(getRegisterName(Reg)).lower();
}

bool E32CInstPrinter::printInst(const MCInst *MI, raw_ostream &OS,
...
static bool usesGivenOffset(const MCInst *MI, int AddOffset) {
  ...
  return LPAC::encodeE32CAluCode(AluCode) == LPAC::ADD &&
...
bool E32CInstPrinter::printMemoryLoadIncrement(const MCInst *MI,
                                                raw_ostream &OS,
                                                StringRef Opcode,
                                                int AddOffset) {
...
bool E32CInstPrinter::printMemoryStoreIncrement(const MCInst *MI,
                                                 raw_ostream &OS,
                                                 StringRef Opcode,
                                                 int AddOffset) {
  if (isPreIncrementForm(MI, AddOffset)) {
...
bool E32CInstPrinter::printAlias(const MCInst * /*MI*/, raw_ostream & /*OS*/) {
  return false;
}

void E32CInstPrinter::printInst(const MCInst *MI, uint64_t Address,
...
void E32CInstPrinter::printOperand(const MCInst *MI, unsigned OpNo,
...
void E32CInstPrinter::printMemImmOperand(const MCInst *MI, unsigned OpNo,
                                          raw_ostream &OS) {
  const MCOperand &Op = MI->getOperand(OpNo);
  if (Op.isImm()) {
    OS << '[' << formatHex(Op.getImm()) << ']';
...
void E32CInstPrinter::printHi16ImmOperand(const MCInst *MI, unsigned OpNo,
                                           raw_ostream &OS) {
  const MCOperand &Op = MI->getOperand(OpNo);
  if (Op.isImm()) {
...
void E32CInstPrinter::printHi16AndImmOperand(const MCInst *MI, unsigned OpNo,
                                              raw_ostream &OS) {
  const MCOperand &Op = MI->getOperand(OpNo);
  if (Op.isImm()) {
...
void E32CInstPrinter::printLo16AndImmOperand(const MCInst *MI, unsigned OpNo,
                                              raw_ostream &OS) {
  const MCOperand &Op = MI->getOperand(OpNo);
  if (Op.isImm()) {
...
static void printMemoryBaseRegister(raw_ostream &OS, const unsigned AluCode,
  ...
  if (LPAC::isPreOp(AluCode))
    OS << "*";
  OS << "%" << E32CInstPrinter::getRegisterName(RegOp.getReg());
...
void E32CInstPrinter::printMemRiOperand(const MCInst *MI, int OpNo,
                                         raw_ostream &OS,
                                         const char * /*Modifier*/) {
...
void E32CInstPrinter::printMemRrOperand(const MCInst *MI, int OpNo,
                                         raw_ostream &OS,
                                         const char * /*Modifier*/) {
  const MCOperand &RegOp = MI->getOperand(OpNo);
  const MCOperand &OffsetOp = MI->getOperand(OpNo + 1);
  ...
  if (LPAC::isPostOp(AluCode))
    OS << "*";
  OS << " " << LPAC::e32cAluCodeToString(AluCode) << " ";
  OS << "%" << getRegisterName(OffsetOp.getReg());
  OS << "]";
}

void E32CInstPrinter::printMemSplsOperand(const MCInst *MI, int OpNo,
                                           raw_ostream &OS,
                                           const char * /*Modifier*/) {
  const MCOperand &RegOp = MI->getOperand(OpNo);
  const MCOperand &OffsetOp = MI->getOperand(OpNo + 1);
...
void E32CInstPrinter::printCCOperand(const MCInst *MI, int OpNo,
                                      raw_ostream &OS) {
  LPCC::CondCode CC =
  ...
  if (CC >= LPCC::UNKNOWN)
    ...
    OS << e32cCondCodeToString(CC);
}

void E32CInstPrinter::printPredicateOperand(const MCInst *MI, unsigned OpNo,
                                             raw_ostream &OS) {
  LPCC::CondCode CC =
      static_cast<LPCC::CondCode>(MI->getOperand(OpNo).getImm());
  // Handle the undefined value here for printing so we don't abort().
