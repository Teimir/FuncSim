//===-- E32CDelaySlotFiller.cpp - E32C delay slot filler ----------------===//
//
...
static cl::opt<bool>
    NopDelaySlotFiller("e32c-nop-delay-filler", cl::init(false),
                       cl::desc("Fill E32C delay slots with NOPs."),
                       cl::Hidden);
...
struct Filler : public MachineFunctionPass {
  ...
  StringRef getPassName() const override { return "E32C Delay Slot Filler"; }

  bool runOnMachineBasicBlock(MachineBasicBlock &MBB);

  bool runOnMachineFunction(MachineFunction &MF) override {
    const E32CSubtarget &Subtarget = MF.getSubtarget<E32CSubtarget>();
...
// createE32CDelaySlotFillerPass - Returns a pass that fills in delay
// slots in E32C MachineFunctions
FunctionPass *
llvm::createE32CDelaySlotFillerPass(const E32CTargetMachine & /*tm*/) {
  return new Filler();
}
  ...
  for (MachineBasicBlock::instr_iterator I = MBB.instr_begin();
    ...
    if (I->getDesc().hasDelaySlot()) {
      ...
      if (I->getOpcode() == E32C::RET) {
        ...
        assert(RI->getOpcode() == E32C::LDW_RI && RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == E32C::SP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == E32C::SP &&
               RI->getOperand(2).isImm() && RI->getOperand(2).getImm() == -8);
        ++RI;
        assert(RI->getOpcode() == E32C::ADD_I_LO &&
        ...
        if (!NopDelaySlotFiller && findDelayInstr(MBB, I, J)) {
          ...
          BuildMI(MBB, std::next(I), DebugLoc(), TII->get(E32C::NOP));
        }
        ++FilledSlots;
      }
