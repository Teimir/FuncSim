//===-- E32CFrameLowering.cpp - E32C Frame Information ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the E32C implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
void E32CFrameLowering::determineFrameLayout(MachineFunction &MF) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  const E32CRegisterInfo *LRI = STI.getRegisterInfo();
...
// ADJDYNALLOC pseudo instructions with a E32C:ADDI with the
// maximum call frame size as the immediate.
void E32CFrameLowering::replaceAdjDynAllocPseudo(MachineFunction &MF) const {
  const E32CInstrInfo &LII =
      *static_cast<const E32CInstrInfo *>(STI.getInstrInfo());
  unsigned MaxCallFrameSize = MF.getFrameInfo().getMaxCallFrameSize();

  for (MachineBasicBlock &MBB : MF) {
    for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
      if (MI.getOpcode() == E32C::ADJDYNALLOC) {
        DebugLoc DL = MI.getDebugLoc();
        Register Dst = MI.getOperand(0).getReg();
        Register Src = MI.getOperand(1).getReg();

        BuildMI(MBB, MI, DL, LII.get(E32C::ADD_I_LO), Dst)
            .addReg(Src)
...
void E32CFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  // Push old FP
  // st %fp,-4[*%sp]
  BuildMI(MBB, MBBI, DL, LII.get(E32C::SW_RI))
      .addReg(E32C::SP)
      .addReg(E32C::SP)
      .addImm(-4)
  ...
  // Generate new FP
  // add %sp,8,%fp
  BuildMI(MBB, MBBI, DL, LII.get(E32C::ADD_I_LO), E32C::SP)
      .addReg(E32C::SP)
      .addImm(8)
      .setMIFlag(MachineInstr::FrameSetup);
  ...
  // sub %sp,StackSize,%sp
  if (StackSize != 0) {
    BuildMI(MBB, MBBI, DL, LII.get(E32C::SUB_I_LO), E32C::SP)
        .addReg(E32C::SP)
        .addImm(StackSize)
        .setMIFlag(MachineInstr::FrameSetup);
  }
...
MachineBasicBlock::iterator E32CFrameLowering::eliminateCallFramePseudoInstr(
...
void E32CFrameLowering::emitEpilogue(MachineFunction & /*MF*/,
                                      MachineBasicBlock &MBB) const {
  ...
  BuildMI(MBB, MBBI, DL, LII.get(E32C::LDW_RI), E32C::SP)
      .addReg(E32C::SP)
      .addImm(-8)
      .addImm(LPAC::ADD);
}

void E32CFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                              BitVector &SavedRegs,
                                              RegScavenger *RS) const {
  TargetFrameLowering::determineCalleeSaves(MF, SavedRegs, RS);

  MachineFrameInfo &MFI = MF.getFrameInfo();
  const E32CRegisterInfo *LRI =
      static_cast<const E32CRegisterInfo *>(STI.getRegisterInfo());
  int Offset = -4;
